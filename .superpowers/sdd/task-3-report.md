...
1. **Tablero** — No code changes needed. `$zMetricsDef = gbe_rol_mv_zona_metrics()` returns 10 keys, `$fMetricsDef = gbe_rol_mv_faccion_metrics()` returns 7 keys. Both foreach loops on lines ~336 and ~364 already iterate dynamically over the metric definitions.
...
4. **NPC tracking diff en vista previa** — Added `<h3 class="mv-prev-h">Tracking de NPCs mayores</h3>` section after threads. Reads `$preview['estado']['npc_tracking']`, gets current tracking via `gbe_rol_mv_npc_tracking_from_db()`, shows salud/moral/ubicación diff with `→` arrows using `.mv-diff-block` / `.mv-diff-line` classes.
