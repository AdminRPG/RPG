<?php
/**
 * Granblue Fantasy: Eternal · Migración de BD para el rename iforge -> ope.
 * ----------------------------------------------------------------
 ...
 *
 *   php scripts/migrate-gbe-db.php
 *
...
$cols = [
    ['posts',   'iforge_pid',     'gbe_pid'],
    ['threads', 'iforge_pid',     'gbe_pid'],
    ['threads', 'iforge_lastpid', 'gbe_lastpid'],
    ['forums',  'iforge_lastpid', 'gbe_lastpid'],
];
...
/* ── 2) datacache: iforge_home -> gbe_home ── */
$db->query("UPDATE {$P}datacache SET title='gbe_home' WHERE title='iforge_home'");
echo "CACHE: iforge_home -> gbe_home (filas: {$db->affected_rows})\n";

/* ── 3) plugins cache: codename iforge_rol -> gbe_rol (serializado) ── */
$res = $db->query("SELECT cache FROM {$P}datacache WHERE title='plugins' LIMIT 1");
if ($row = $res->fetch_assoc()) {
    $data = @unserialize($row['cache']);
    if (is_array($data)) {
        $data = deep_replace($data, 'iforge_rol', 'gbe_rol');
        $ser = $db->real_escape_string(serialize($data));
        $db->query("UPDATE {$P}datacache SET cache='{$ser}' WHERE title='plugins'");
        echo "PLUGINS: codename iforge_rol -> gbe_rol\n";
    }
}

/* ── 4) themestylesheets: iforge.css -> gbe.css ── */
$db->query("UPDATE {$P}themestylesheets SET name='gbe.css' WHERE name='iforge.css'");
echo "STYLESHEET: name iforge.css -> gbe.css (filas: {$db->affected_rows})\n";
...
if ($row = $res->fetch_assoc()) {
    ...
    if (is_array($data)) {
        ...
        if (isset($data['properties']) && is_string($data['properties'])) {
            ...
            if (is_array($inner)) {
                $inner = deep_replace($inner, 'iforge.css', 'gbe.css');
        ...
        echo "DEFAULT_THEME: rutas iforge.css -> gbe.css\n";
    }
}
