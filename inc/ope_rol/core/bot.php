 ...
 * Necesita un personaje-sistema en mybb_rol_personajes (es_npc) para firmar
 * los posts como personaje (el postbit muestra el personaje, no la cuenta).
 */
...
/**
 * Asegura (idempotente) el personaje-sistema del bot en mybb_rol_personajes.
 * @return int pid del personaje del bot (0 si falla).
 */
function ope7_bot_personaje()
{
    ...
    // Crear la persona del bot (foto fija, sin progreso).
    $now = TIME_NOW;
...
function ope7_bot_post_thread($fid, $subject, $message, $tag = '')
{
    ...
    if ($tid > 0 && function_exists('ope_rol_store_thread_meta') && $db->table_exists('rol_thread_meta')) {
