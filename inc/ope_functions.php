<?php
/**
 * One Piece: Eternal · Helpers compartidos (index.php + forumdisplay.php)
 * --------------------------------------------------------------
...
if (!function_exists('ope_skydom_image')) {
    /** Arte de panel regional por slug estable (images/ope/skydom-*.jpg — legado visual). */
    function ope_skydom_image($slug) {
        global $mybb;
        static $cache = [];
        ...
        $file = 'skydom-'.$slug.'.jpg';
        $rel = 'images/ope/'.$file;
        if (@is_file(MYBB_ROOT.$rel)) {
            return $cache[$slug] = $mybb->settings['bburl'].'/'.$rel;
        }
...
if (!function_exists('ope_region_art_html')) {
    function ope_region_art_html($slug, $fid, $alt) {
        $img = ope_skydom_image($slug);
        if ($img === null) {
            $img = ope_forum_image($fid);
        }
        if ($img !== null) {
            return '<img src="'.$img.'" alt="'.htmlspecialchars_uni($alt).'" loading="lazy">';
        }
        return ope_sector_art($fid);
    }
}

if (!function_exists('ope_sector_art')) {
    /** Arte SVG procedural determinista por fid (fallback cuando no hay foto).
     *  Estética OPE: cielo, nubes, éter, islas flotantes. */
...
if (!function_exists('ope_render_region_cards')) {
    ...
    function ope_render_region_cards($pid, array $forumpermissions) {
        ...
        while ($forum = $db->fetch_array($forumQuery)) {
            ...
            $slug = trim($slug, '-');
            $art = ope_region_art_html($slug, (int)$forum['fid'], $forum['name']);
