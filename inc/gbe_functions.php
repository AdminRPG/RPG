 ...
 *   - gbe_reltime()          tiempo relativo ("hace 5 min")
 *   - gbe_heat()             badge de calor E..SS según nº de posts
 *   - gbe_forum_image()      localiza images/foros/{fid}.{ext}
 *   - gbe_sector_art()       arte SVG procedural (fallback sin foto)
 *   - gbe_render_region_cards()  tarjetas grandes con foto para los
 *                                    hijos directos de un foro/categoría
 *   - gbe_world_root_name()  nombre de la categoría raíz de un foro,
...
if (!function_exists('gbe_reltime')) {
    function gbe_reltime($ts) {
        $d = TIME_NOW - (int)$ts;
        if ($d < 0) $d = 0;
...
if (!function_exists('gbe_heat')) {
    function gbe_heat($posts) {
        $posts = (int)$posts;
...
if (!function_exists('gbe_forum_image')) {
    function gbe_forum_image($fid) {
        global $mybb;
        static $cache = [];
        $fid = (int)$fid;
...
if (!function_exists('gbe_render_region_cards')) {
    /**
     ...
     * hijos son sus islas).
     */
    function gbe_render_region_cards($pid, array $forumpermissions) {
        global $db;
        ...
        while ($forum = $db->fetch_array($forumQuery)) {
            ...
            $img = gbe_forum_image($forum['fid']);
            $art = $img !== null
                ? '<img src="'.$img.'" alt="'.$forumName.'" loading="lazy">'
                : gbe_sector_art($forum['fid']);
            ...
            $descHtml = $forumDesc !== '' ? '<div class="gbe-region-d">'.$forumDesc.'</div>' : '';
            ...
            $cards .= '
            <a href="'.$bburl.'/forumdisplay.php?fid='.$forum['fid'].'" class="gbe-region gbe-region--'.$slug.'">
                <div class="gbe-region-art">'.$art.'</div>
                <div class="gbe-region-veil"></div>
                <div class="gbe-region-in">
                    <div class="gbe-region-n">'.$forumName.'</div>
                    '.$descHtml.'
                    <div class="gbe-region-m">'.$meta.'</div>
                </div>
            </a>';
        }
        return $cards;
...
if (!function_exists('gbe_forum_meta')) {
    /**
...
if (!function_exists('gbe_world_root_name')) {
    /**
     ...
     */
    function gbe_world_root_name(array $foruminfo) {
        global $db;
