 ...
 * Formulario multi-paso: origen, destino, barco, tripulación → GBEternal crea el hilo.
...
if ($loggedin && $mybb->request_method === 'post' && $mybb->get_input('action') === 'cerrar') {
    ...
    $res = gbe_viaje_cerrar($vid, $uid, $active_pid);
    if ($res['ok']) {
...
$islas = function_exists('gbe_viaje_islas') ? gbe_viaje_islas() : array();
$barcos = function_exists('gbe_oraculo_barcos_config') ? gbe_oraculo_barcos_config() : array();
$viaje_activo = ($active_pid > 0 && function_exists('gbe_viaje_por_capitan_activo')) ? gbe_viaje_por_capitan_activo($active_pid) : null;

// Compañeros de tripulación (misma crew)
$crew_pids = array();
if ($active_pid > 0 && function_exists('gbe_rol_cat_tripulacion_de_personaje')) {
    $mi = gbe_rol_cat_tripulacion_de_personaje($active_pid);
...
<body class="gbe-pg-tramites gbe-pg-viajes">
...
<div class="wrap">
  <section class="reveal vis">
    ...
    <p class="gbe-vj-lead">Traza una travesía entre islas y deja que <strong>Lyria</strong> lea el cielo: clima, encuentros, hallazgos y peligros formarán el hilo de <em>Alta Mar</em>. La tripulación decide cuánto dura la historia: <strong>el viaje solo termina cuando solicitas la llegada</strong>.</p>
    <div class="gbe-vj-facts" aria-label="Características del viaje">
  ...
  <?php if ($flash !== ''): ?>
  <div class="gbe-flash gbe-flash--<?php echo $flash_kind === 'error' ? 'err' : 'ok'; ?>"><?php echo htmlspecialchars_uni($flash); ?></div>
  <?php endif; ?>

  <?php if (!$loggedin || $active_pid < 1): ?>
  <div class="gbe-plate gbe-vj-gate">
    <div class="gbe-plate-b">
      <p>Necesitas iniciar sesión y tener un <strong>personaje activo</strong> para solicitar un viaje.</p>
      <a href="<?php echo $bburl; ?>/member.php?action=login" class="gbe-btn gbe-btn-hot">Entrar</a>
    </div>
  </div>
  <?php elseif ($viaje_activo): ?>
  <div class="gbe-plate gbe-vj-active">
    <div class="gbe-plate-h"><span class="gbe-plate-h-t">Viaje en curso</span></div>
    <div class="gbe-plate-b">
      ...
      <a href="<?php echo $bburl; ?>/showthread.php?tid=<?php echo (int) $viaje_activo['tid']; ?>" class="gbe-btn gbe-btn-hot">Ir al hilo de travesía</a>
    </div>
  </div>
  <?php else: ?>

  <div class="gbe-vj-layout">
    <figure class="gbe-vj-aside<?php echo $hero_image_url !== '' ? ' has-image' : ''; ?>">
      ...
      <?php else: ?>
      <div class="gbe-vj-aside-placeholder" aria-hidden="true">
  ...
  <form class="gbe-vj-wizard" id="gbe-vj-form" method="post" action="<?php echo $bburl; ?>/viajes.php">
    <input type="hidden" name="my_post_key" value="<?php echo $mybb->post_code; ?>">
    <input type="hidden" name="action" value="solicitar">

    <div class="gbe-vj-steps" role="tablist">
      <button type="button" class="gbe-vj-step is-on" data-step="1" role="tab" aria-selected="true"><span>01</span>Ruta</button>
      <button type="button" class="gbe-vj-step" data-step="2" role="tab" aria-selected="false"><span>02</span>Barco</button>
      <button type="button" class="gbe-vj-step" data-step="3" role="tab" aria-selected="false"><span>03</span>Tripulación</button>
      <button type="button" class="gbe-vj-step" data-step="4" role="tab" aria-selected="false"><span>04</span>Confirmar</button>
    </div>

    <!-- Paso 1: Ruta -->
    <div class="gbe-vj-panel is-on" data-panel="1" role="tabpanel">
      <div class="gbe-vj-panel-head">
        <h2>Traza la ruta</h2>
        <p>El origen y el destino determinan la distancia, los tramos y cuántas veces hablará el oráculo.</p>
      </div>
      <div class="gbe-vj-route-grid">
        <div class="gbe-vj-field">
          <label for="fid_origen">Origen</label>
          <select name="fid_origen" id="fid_origen" required class="gbe-vj-sel">
        ...
        <div class="gbe-vj-route-arrow" aria-hidden="true">→</div>
        <div class="gbe-vj-field">
          <label for="fid_destino">Destino</label>
          <select name="fid_destino" id="fid_destino" required class="gbe-vj-sel">
      ...
      <div class="gbe-vj-preview" id="gbe-vj-route-preview">
        <span class="gbe-vj-preview-k">Tramos estimados</span>
        <span class="gbe-vj-preview-v" id="gbe-vj-tramos">—</span>
      </div>
      <button type="button" class="gbe-btn gbe-btn-hot gbe-vj-next" data-next="2">Continuar</button>
    </div>

    <!-- Paso 2: Barco -->
    <div class="gbe-vj-panel" data-panel="2" role="tabpanel" hidden>
      <div class="gbe-vj-panel-head">
      ...
      <div class="gbe-vj-field">
        ...
        <input type="text" name="barco_nombre" id="barco_nombre" required maxlength="120" class="gbe-vj-inp" placeholder="p. ej. Going Merry">
      </div>
      <div class="gbe-vj-barco-grid">
        <?php foreach ($barcos as $key => $b): ?>
        <label class="gbe-vj-barco-card">
          <input type="radio" name="barco_tipo" value="<?php echo htmlspecialchars_uni($key); ?>" <?php echo $key === 'estandar' ? 'checked' : ''; ?>>
          <span class="gbe-vj-barco-in">
      ...
      <div class="gbe-vj-field">
        ...
        <input type="text" name="suministros" id="suministros" class="gbe-vj-inp" placeholder="Comida, agua, madera…">
      </div>
      <div class="gbe-vj-nav-row">
        <button type="button" class="gbe-btn gbe-btn-ghost gbe-vj-prev" data-prev="1">Atrás</button>
        <button type="button" class="gbe-btn gbe-btn-hot gbe-vj-next" data-next="3">Continuar</button>
      </div>
    </div>

    <!-- Paso 3: Tripulación -->
    <div class="gbe-vj-panel" data-panel="3" role="tabpanel" hidden>
      <div class="gbe-vj-panel-head">
      ...
      <p class="gbe-vj-note">Capitán: <strong><?php echo htmlspecialchars_uni($pj_nombre); ?></strong> (Navegante). Marca compañeros de tu tripulación a bordo:</p>
      <?php if ($companeros): ?>
      <div class="gbe-vj-crew">
        <?php foreach ($companeros as $c): ?>
        <label class="gbe-vj-crew-chip">
      ...
      <?php else: ?>
      <p class="gbe-vj-empty">Viajas solo o sin tripulación registrada. Los oficios extra no aplicarán modificadores.</p>
      <?php endif; ?>
      <div class="gbe-vj-field">
        <label for="notas">Notas para el narrador (opcional)</label>
        <textarea name="notas" id="notas" rows="3" class="gbe-vj-ta" placeholder="Ruta preferida, carga especial…"></textarea>
      </div>
      <div class="gbe-vj-nav-row">
        <button type="button" class="gbe-btn gbe-btn-ghost gbe-vj-prev" data-prev="2">Atrás</button>
        <button type="button" class="gbe-btn gbe-btn-hot gbe-vj-next" data-next="4">Revisar</button>
      </div>
    </div>

    <!-- Paso 4: Confirmar -->
    <div class="gbe-vj-panel" data-panel="4" role="tabpanel" hidden>
      <div class="gbe-vj-panel-head">
      ...
      <div class="gbe-vj-summary" id="gbe-vj-summary"></div>
      <p class="gbe-vj-warn">Al confirmar, Lyria crear&aacute; el hilo con el Or&aacute;culo resuelto. Podr&eacute;is rolear hasta que <strong>t&uacute;</strong> solicites la llegada.</p>
      <div class="gbe-vj-nav-row">
        <button type="button" class="gbe-btn gbe-btn-ghost gbe-vj-prev" data-prev="3">Atrás</button>
        <button type="submit" class="gbe-btn gbe-btn-hot gbe-vj-submit">Invocar Oráculo y zarpar</button>
      </div>
    </div>
  </form>
  </div>
...
(function(){
  ...
  var form = document.getElementById('gbe-vj-form');
  if (!form) return;
  ...
  var trEl = document.getElementById('gbe-vj-tramos');
  function updRoute() {
  ...
  function goStep(n) {
    form.querySelectorAll('.gbe-vj-step').forEach(function(s){
    ...
    form.querySelectorAll('.gbe-vj-panel').forEach(function(p){
  ...
  form.querySelectorAll('.gbe-vj-next').forEach(function(b){
    b.addEventListener('click', function(){
      var cur = b.closest('.gbe-vj-panel');
  ...
  form.querySelectorAll('.gbe-vj-prev').forEach(function(b){ b.addEventListener('click', function(){ goStep(b.getAttribute('data-prev')); }); });
  form.querySelectorAll('.gbe-vj-step').forEach(function(s){ s.addEventListener('click', function(){ goStep(s.getAttribute('data-step')); }); });

  function buildSummary() {
    var box = document.getElementById('gbe-vj-summary');
    if (!box) return;
    var t = calcTramos(oSel.value, dSel.value);
    var pp = postsFor(t);
    var barco = form.querySelector('[name="barco_nombre"]').value || '—';
    var tipo = form.querySelector('[name="barco_tipo"]:checked');
    box.innerHTML = '<div class="gbe-vj-sum-row"><span>Ruta</span><b>' + islaName(oSel.value) + ' → ' + islaName(dSel.value) + '</b></div>'
      + '<div class="gbe-vj-sum-row"><span>Tramos / posts sug.</span><b>' + t + ' · ' + pp[0] + ' posts</b></div>'
      + '<div class="gbe-vj-sum-row"><span>Barco</span><b>' + barco + (tipo ? ' (' + tipo.value + ')' : '') + '</b></div>';
  }
  ...
  form.addEventListener('submit', function(e){
    ...
    if (window.gsap) gsap.to('.gbe-vj-submit', {scale:0.96, duration:0.1, yoyo:true, repeat:1});
  });
})();
</script>
</body>
