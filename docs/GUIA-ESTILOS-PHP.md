...
> ⚠️ **GOTCHA nº1** (causa de páginas rotas): las clases base **`.shead .plate .plate-h .plate-b .reveal .flash .pj-empty` NO son globales**. Cada página las **re-declara** bajo su scope. Scaffolding canónico OPE: **DESIGN §5.4** (no el brutalismo OP con `border:2px solid #000`). SÍ son globales: `.wrap .breadcrumb .btn* .ope-prog-*`.
