...
| **Marca antigua / codename** | One Piece Eternal (`ope`) + I-Forge (`iforge`) | **PURGAR** todo rastro del código, DB y docs |
...
El codename actual es `ope` (One Piece Eternal). El nuevo codename recomendado: **`sky`** (corto, neutro, ownable). Se usará como:
...
| Eje | One Piece Eternal (antiguo) | Skyfarers Eternal (nuevo) |
|---|---|---|
...
| **Ether / Magoi** | Energía mágica | Reserva de combate | EN (`ope_combat_calc_en`) |
...
| **Bounty / Wanted** | Progresión visible, estatus, rivalidad | **Renombre de Skyfarer** (fama por Skydom) | `ope_rol_wanted.php` → `sky_rol_renombre.php` |
...
| **Escala de poder / rangos** | Jerarquía clara aspiracional | Rangos de Skyfarer (Rookie→Legend) | `ope_rol_nivel_label()` reskin |
...
| **Combate con reglas claras** | Justicia percibida, anti-godmod | PV/EN/PA + heridas + snapshots | `ope_rol_system.php` |
...
|---|---|---|---|
| `ope_` / `ope-` / `OPE_` | Codename One Piece Eternal (funciones, CSS, constantes, datacache) | **Cientos de ocurrencias en ~60 archivos PHP + CSS + XML** | Renombrar → `sky_` |
...
| `One Piece` / `One Piece Eternal` | Nombre de marca en copy, docs, bot | `bbname`, docs, guías, bot `OP-Eternal` | Reemplazar por marca nueva |
...
- **Catálogos:** `inc/ope_rol_data.php` (razas, facciones, armas, packs), `inc/ope_rol_catalogos.php`
- **Sistemas de poder:** `inc/ope_rol_haki.php`, `haki.php`, `inc/ope_rol_wanted.php`, `inc/ope_rol_pl.php`
...
- **Mundo:** `inc/ope_rol_mundo.php`, `inc/ope_rol_viajes.php`

### 5.3 Design docs (estado tras purga Jul 2026)

**Fuente de verdad:** `docs/DESIGN-GRANBLUE-ETERNAL.md`, `docs/themes/ope.css`, `docs/themes/README.md`.
...
### 6.1 Codename PHP/CSS: `ope_` → `sky_`

Reemplazo masivo ordenado (reutilizar el patrón de `scripts/rename-ope.php`):

```
ope_        → sky_         # funciones, globales, claves de caché, columnas de codename
ope-        → sky-         # clases CSS, prefijos de plantilla (ope-pg-* → sky-pg-*)
OPE_        → SKY_         # constantes
ope.css     → sky.css      # hoja de estilos
...
4. **Renombrar archivos físicos** cuyo nombre lleva codename:
   - `inc/ope_rol_*.php` → `inc/sky_rol_*.php` (13+ archivos)
   - `inc/ope_functions.php` → `inc/sky_functions.php`
   - `inc/ope_user_init.php` → `inc/sky_user_init.php`
   - `docs/themes/ope*.xml` → `docs/themes/sky*.xml`
   - `docs/themes/ope.css` → `docs/themes/sky.css`
   - `inc/plugins/ope_rol.php` → `inc/plugins/sky_rol.php` (⚠️ actualizar registro del plugin en MyBB)
5. Actualizar `require_once`/`include` que apunten a rutas renombradas (grep `ope_rol_`, `ope_functions`, `ope_user_init`).
...
1. **Crear** `scripts/rebrand-sky.php` (clon de `rebrand-opeternal.php`):
   ...
   - datacache `ope_home` → `sky_home` con curiosidades/lore Granblue.
2. **Bot sistema:** renombrar usuario/PJ `OP-Eternal` → `Lyria` (BD `users` + `rol_personajes`, ver `ope_system_uid()`/`ope_system_pid()` → `sky_system_*`).
...
| De | A |
|---|---|
| `inc/ope_rol_data.php` | `inc/sky_rol_data.php` (+ reescribir catálogos) |
| `inc/ope_rol_system.php` | `inc/sky_rol_system.php` |
| `inc/ope_rol_haki.php` | `inc/sky_rol_clases.php` |
| `inc/ope_rol_wanted.php` | `inc/sky_rol_renombre.php` |
| `inc/ope_rol_pl.php` | `inc/sky_rol_cristales.php` |
| `inc/ope_rol_mundo.php` | `inc/sky_rol_mundo.php` |
| `inc/ope_rol_viajes.php` | `inc/sky_rol_viajes.php` |
| `inc/ope_rol_catalogos.php` | `inc/sky_rol_catalogos.php` |
| `inc/ope_rol_oraculo*.php` | `inc/sky_rol_oraculo*.php` |
| `inc/ope_functions.php`, `inc/ope_user_init.php` | `inc/sky_*` |
| `inc/plugins/ope_rol.php` | `inc/plugins/sky_rol.php` (⚠️ re-registrar plugin) |
| `haki.php` | `clases.php` |
| `docs/themes/ope*.xml`, `ope.css` | `sky*.xml`, `sky.css` |
...
- `datacache`: `ope_home` → `sky_home`.
- `settings`: `bbname`.
- `users` + `rol_personajes`: bot `OP-Eternal` → `Lyria`.
...
rg "ope_|ope-|OPE_" --glob '*.php' --glob '*.css' --glob '*.xml'
