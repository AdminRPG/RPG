...
- **Correcciones menores:** mensaje vacío de narradores decía «staff_narrador en rol_cuentas» — corregido a `rol_personajes` (21.2). CSS: `.zs-tab` añadido al scope de facciones y `.zs-ficha`/`.zs-bloque` al de akumas; el resto reutiliza clases existentes (`mv-*`, `zs-*`, `ms-*`).
...
### F6.3 · Retirada por capas de `mybb_rol_*` (2026-08-28)
- **Decisión (consultada, opción recomendada):** retirada **por capas** — se migra primero todo lo que el motor 7 Seas toca en vivo y se retira/archiva solo lo huérfano; el plugin viejo (4.500 líneas, da navbar/head/postbit a todas las páginas) se **conserva intacto** con sus guards `table_exists()` como legado documentado, hasta una futura reescritura. Nada se DROP: las tablas se archivan con `RENAME TABLE mybb_rol_X TO mybb_rol_retirada_X` (reversible).
- **`mybb_ope_cuentas` creada** (A.1, sustituye `rol_cuentas` como fuente canónica): puntero de personaje activo + roles staff/narrador + slots. Migración idempotente de datos: `INSERT ... SELECT ... ON DUPLICATE KEY UPDATE` desde `mybb_rol_cuentas` (solo si la tabla legada existe).
- **Código migrado a la fuente canónica** (con fallback al legado si la tabla nueva no existe): `ope7_pj_activo`/`ope7_pj_set_activo` (`dominio/personajes.php`, escribe en `ope_cuentas` + espejo en `rol_cuentas` para el plugin) · `ope7_permisos` (`core/permisos.php`, staff/narrador leen de `ope_cuentas`, sin depender de `ope_rol_active_staff` del plugin) · `ope7_bot_personaje` (`core/bot.php`, bot «OPE Eternal» en `ope_personajes` uid=2 `es_NPC=1`) · narradores de misiones (join `ope_cuentas`/`ope_personajes`) · bibliotecas públicas `ope_rol_cat_personajes_publicos`/`ope_rol_cat_npcs_publicos`/`ope_rol_cat_fichas_query` (`catalogos/gestion.php`, leen de `ope_personajes` con join de razas/facciones).
- **Bug real corregido en la migración:** `ope7_tabla_existe_legacy` concatena `mybb_rol_` + nombre, así que la llamada con `'rol_cuentas'` buscaba `mybb_rol_rol_cuentas` (doble prefijo) y la copia de datos nunca se ejecutaba — corregido a `'cuentas'`. Detectado porque `ope_cuentas` quedó vacía tras la primera corrida.
- **20 tablas archivadas** (solo código muerto del motor viejo, todas vacías o con datos residuales de pruebas): `barco_cofre_logs`, `barco_invitaciones`, `barcos`, `calendario`, `flash`, `misiones`, `mv_arcos`, `mv_audit`, `mv_ciclos`, `mv_eventos`, `mv_facciones`, `mv_misiones`, `mv_npc_menores`, `mv_tension`, `mv_zonas`, `nav_items`, `tiradas`, `transacciones`, `viajes`, `viajes_cola`. **34 conservadas como legado** (las usan hooks activos del plugin o `gestion.php` vivo): `rol_cuentas`, `rol_personajes`, `rol_post_snapshot`, `rol_thread_snapshots`, `rol_mision_tomas`, `rol_thread_meta`, `rol_npcs_secundarios`, `rol_tecnicas`, `rol_tienda_items`, `rol_lore`, `rol_akuma`, `rol_bestiario`, `rol_estados`, `rol_cards`, `rol_pj_cards`, `rol_pj_fruta`, `rol_haki`, `rol_estilos`, `rol_enlace`, `rol_pp_saldo`, `rol_pp_log`, `rol_tramites`, `rol_tripulaciones`, `rol_tripulacion_miembros`, `rol_pj_vocaciones`, `rol_pl`, `rol_pl_log`, `rol_rachas`, `rol_renombre`, `rol_mv_mision_asignaciones`, `rol_mv_noticias` (la portada la lee), `rol_cartas`, `rol_epoch`, `rol_edades` (las 2 últimas con referencia muerta con guard).
