...
10. [Patrones de reskin (OP → GBF)](#10-patrones-de-reskin-op--gbf)
...
Portar un prototipo (`docs/Prototypes/Granblue/`) = implementar **todas** las capas del árbol (§6.1), no un componente aislado. El incidente jul-2026: carrusel GBF + tablón OP → producción no igual al prototipo.
...
Tema emocional: de *Libertad* (mar, OP) → **Horizonte** (cielo, GBF). La escena de referencia: amanecer desde la cubierta de una aeronave sobre un mar de nubes doradas, islas recortadas contra cielo índigo-turquesa.
...
3. **Cada elemento tiene referente GBF.** Éter, Primals, aeronaves, Skydoms.
...
#### Elementos GBF (`body[data-element]` en ficha)
...
#### Facciones GBF (reemplazan `--fac-pirata`…)
...
| Uso | Estilo |
|---|---|
| CTA primario (`.gbe-btn-hot`, `.btn-hot`) | Rectangular, `border-radius: 8px`, gradiente oro, borde sutil, sombra `--gbe-shadow-sm` |
| Secundario (`.gbe-btn-ghost`) | Fondo `--gbe-card`, borde `--gbe-line`, texto `--gbe-ink` |
...
Prohibido en UI GBF: pills con glow exagerado, `#5865F2` sólido tipo app móvil, `border-radius: 30px` en CTAs.

```css
/* Patrón canónico portada / páginas GBF */
.gbe-btn{border-radius:8px;padding:10px 22px;font-family:var(--mark);text-transform:none}
```
...
body.gbe-pg-<pagina> .shead h1,body.gbe-pg-<pagina> .shead h2{font-family:var(--disp);font-weight:800;font-size:2rem;color:var(--paper);line-height:1}
body.gbe-pg-<pagina> .shead .code{font-family:var(--mono);font-size:.7rem;font-weight:700;color:var(--ember-hi);letter-spacing:1px}
body.gbe-pg-<pagina> .shead .rule{flex:1;height:2px;background:repeating-linear-gradient(90deg,var(--rivet) 0 6px,transparent 6px 12px)}
body.gbe-pg-<pagina> .plate{border:1px solid var(--line);background:var(--iron-plate);margin-bottom:12px;box-shadow:var(--shadow-sm);border-radius:14px;overflow:hidden}
body.gbe-pg-<pagina> .plate-h{background:linear-gradient(120deg,var(--concrete),var(--iron-plate));padding:12px 16px;display:flex;align-items:center;justify-content:space-between;gap:8px;border-bottom:1px solid var(--line-soft)}
body.gbe-pg-<pagina> .plate-h .t{font-family:var(--disp);font-weight:700;font-size:1rem;color:var(--gold-deep)}
body.gbe-pg-<pagina> .plate-h .c{font-family:var(--mono);font-size:.58rem;font-weight:700;text-transform:uppercase;color:var(--ash)}
body.gbe-pg-<pagina> .plate-b{padding:16px}
body.gbe-pg-<pagina> .flash{border:1px solid var(--line);padding:11px 14px;margin-bottom:16px;font-family:var(--mono);font-size:.74rem;font-weight:700;border-radius:8px}
body.gbe-pg-<pagina> .flash.ok{background:color-mix(in srgb,var(--patina) 20%,transparent);color:var(--paper)}
body.gbe-pg-<pagina> .flash.error{background:var(--crack);color:#fff}
body.gbe-pg-<pagina> .pj-empty{border:1px dashed var(--rivet);background:var(--iron-plate);padding:40px 22px;text-align:center;font-family:var(--mono);font-size:.8rem;color:var(--paper-dim);border-radius:12px}
body.gbe-pg-<pagina> .reveal{opacity:0;transform:translateY(14px);transition:opacity .5s,transform .5s}
body.gbe-pg-<pagina> .reveal.vis{opacity:1;transform:none}
@media(prefers-reduced-motion:reduce){body.gbe-pg-<pagina> .reveal{opacity:1;transform:none}}
```
...
### 6.7 Estado F2b — brecha vs prototipo (julio 2026)

El portado a MyBB **no es copiar el carrusel**: el prototipo es un sistema visual completo. Hasta cerrar F2b, `localhost/iforge/` puede verse “a medias”.
...
## 7. Tríada visual del personaje

Cada Skyfarer tiene **tres assets propios**. Configurables por el dueño en Gestión (`ficha.php`).

| Asset | Campo BD | Uso | Ratio |
|---|---|---|---|
| **Avatar** | `rol_personajes.avatar` | Retrato en ficha (72×72 en v4), selector de personaje, cards | 3:4 o cuadrado |
| **Banner** | `datos.banner` (JSON en `rol_personajes.datos`) | Cabecera full-bleed de la ficha | **16:9** (1920×1080 recomendado) |
| **Icono** | `rol_personajes.icono` | Posts, feed portada, mapa de relaciones, chips | 1:1 (128×128) |

**Implementación actual:** `ficha.php` L109–124 (guardado), L495–496 (lectura), L820–829 (banner), L839–840 (avatar), L1289–1324 (icono en relaciones).

> **NO confundir:** `images/gbe/hero-*.jpg` son decoración del **sitio**. El banner de cada PJ es URL propia (Imgur, CDN…) en `datos.banner`. El prototipo usa `hero-pueblos.jpg` solo como demo.
...
### 8.3 Campos visibles en cabecera (GBF)
...
## 10. Patrones de reskin (OP → GBF)

### 10.1 Los 5 patrones de diseño

| Patrón OP (legado) | Reskin GBF | Página / componente |
|---|---|---|
...
Toggle en navbar: alterna `document.documentElement.dataset.theme` entre `cielo` y `noche`. Persistir en `localStorage` (`gbe-theme`) al portar a MyBB.
...
*Última actualización: julio 2026 — v1.1 + §6.7 estado F2b.*
