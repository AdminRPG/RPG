...
>
> **Estado:** v3.1 — **F2b en progreso** (portada GBF parcialmente alineada; ficha v4 portada). Prototipos HTML cerrados. Purga codename (F1) pendiente.
>
...
| Documentación | `DESIGN-GRANBLUE-ETERNAL.md`, `PRODUCT.md` GBF | Sustituye docs OP |
| Referencia web | Captura Relink + `resumen.md` | Dirección claro/acuarela |
| **Prototipo index v3.2** | Hero-carrusel · Gaceta bento · Skydoms · Off Topic · censo | `docs/Prototypes/Granblue/index.html` |
...
- Listado de islas en portada — las islas viven **dentro** del Skydom (subforos), no en el índice
- Retrato grande 240×300 en ficha — sustituido por banner 16:9 + avatar pequeño

---

## 0.3 Identidad visual del personaje (TRÍADA — cerrado)

Cada Skyfarer tiene **tres assets propios**, configurables por el dueño en Gestión (`ficha.php`):

| Asset | Campo BD | Uso | Ratio recomendado |
|---|---|---|---|
| **Avatar** | `rol_personajes.avatar` | Retrato en ficha (columna/chip), selector de personaje | 3:4 o cuadrado |
| **Banner** | `datos.banner` (JSON) | Cabecera de la ficha, **16:9**, custom por PJ | **16:9** (ej. 1920×1080) |
| **Icono** | `rol_personajes.icono` | Posts, feed, mapa de relaciones, chips pequeños | 1:1 (ej. 128×128) |
...
| # | Decisión | Resolución |
|---|---|---|
| A1 | Codename técnico | **`gbe`** (Granblue Eternal). `ope`/`iforge` → `gbe` |
| A2 | Comunidad | **Reinicio limpio** (truncar PJs OP, empezar de cero) |
| A3 | Orden | **Visual primero** (F2 antes que datos) |
| D1 | Combate | Reskin del motor **+ mecánicas-firma GBF**, con **narrativa apoyada en mecánicas** |
| C2 | Poder único | **Pacto Primal NO universal** (raro, in-game). Universal = **elemento + arma a elección** |
| B1 | Skydoms | **Canónicos de GBF** (Phantagrande/Zegagrande/Nalhegrande…) |
| B2 | Islas | **Originales nuestras** (historia y conflictos propios) |
| B3 | Personajes canónicos | **No aparecen** (universo GBF sin su elenco; Lyria solo como bot/lore) |
| B4 | Época | **Presente alternativo**, línea temporal propia e independiente |
| B5 | Moneda | **Rupies** |
| C1 | Razas | **Human, Erune, Draph, Harvin, Draconic** (sin raza Primal jugable) |
| C3 | Elemento en combate | **Mecánica fuerte** (triángulo GBF) que justifica la narrativa |
| C4 | Elemento | **Fijo**, elegido en creación (define identidad del PJ) |
| W | Arma | **Tipo de arma da bonus y define técnicas** disponibles |
| P | Pacto Primal | **Solo in-game** (pactar/derrotar Primal en trama, aprobado por staff) |
| E1 | Paleta | **La más fiel a Granblue** (ver §4.1) + instalar librerías para subir el nivel visual |
...
### 1.1 Qué es Granblue Fantasy: Eternal

Un foro de rol PBP donde cada jugador es un **Skyfarer**: un aventurero que surca un cielo infinito de islas flotantes a bordo de una aeronave, persiguiendo un sueño. El mundo, la terminología y la mitología son las de **Granblue Fantasy** (Skydoms, Primal Beasts, las razas, Estalucia como tierra prometida), pero **la historia, la cronología y todos los personajes son originales** — igual que un foro de One Piece usa el mundo de Oda sin jugar con Luffy.

### 1.2 Por qué Granblue es el lienzo perfecto para nuestro motor

| Lo que ya tenemos (motor OP) | Encaja en Granblue como |
|---|---|
| Combate PV/EN/PA + heridas + snapshots | Combate de Skyfarers (igual de riguroso) |
| Tripulación + barco | Crew + **aeronave** (pilar central de GBF) |
| Fruta del Diablo (poder único) | **Pacto con Primal Beast** (poder único, más flexible) |
| Haki (progresión por tiers) | **Clases / Jobs** (identidad de build de GBF) |
| Wanted/Bounty | **Renombre de Skyfarer** (fama por Skydom) |
| Facciones | Imperio / Sociedad / Órdenes / Gremios |
| Mundo Vivo "La Balanza" | El equilibrio del cielo entre Astrales, Primals y humanos |
| Escala de poder por rangos | Rangos de Skyfarer (Novato → Leyenda) |

### 1.3 El diferencial

- **CSS-first** (heredado): funciona sin imágenes, identidad en el sistema de tokens.
- **Automatización real**: creación de PJ, combate calculado, snapshots anti-godmod, mundo vivo — algo que casi ningún foro de GBF hispano tiene.
- **Nicho vacío**: apenas hay foros de rol serios de Granblue en español con sistema de combate propio.
...
### 2.1 Personalización
- Wizard de creación (`crear-personaje.php`) → razas GBF, clases, elemento, pacto, virtudes/defectos.
- 12 stats en 3 pilares (Cuerpo/Mente/Espíritu) → se conservan; se re-etiquetan si procede.
- Ficha como **carta de Skyfarer**: **banner 16:9** + avatar + icono (tríada por PJ, §0.3).
...
- Arco maestro de largo plazo: el secreto de los Astrales / Estalucia.

### 2.4 Misiones
- Tablón de contratos (`tablon-misiones.php`, patrón §8.5 del DESIGN) → **Órdenes de Gremio**.
- Tiers T1–T5 por Skydom (dificultad creciente hacia el Alto Cielo).
...
3. **Cada elemento tiene un referente en GBF.** Éter, Primals, aeronaves, Skydoms.
...
:root{
  ...
  --rivet:#3f4d86;
  /* Éter / turquesa (acción secundaria, brillos) */
...
- Quitar/reskin: berries→rupies, wanted→renombre, haki→clase, fruta→pacto (opcional), facción OP→gremio/crew.
- Gestión dueño: avatar + icono + banner (ya en `ficha.php`).

### 4.2b Tipografía legacy (referencia pre-prototipo)

| Rol | Variable | OP actual | **GBF: Eternal** |
|---|---|---|---|
| Display / títulos | `--disp` | Big Shoulders Display | **Cinzel** (serif épico, remates de marca GBF) |
...
> **Iconografía:** SVG inline lineal (se conserva la regla del DESIGN). Motivos GBF: brújula celeste, ala, cristal de Éter, sello de Primal.

### 4.6 Flujo de imágenes decorativas (CONVENCIÓN PERMANENTE)

Granblue vive de su arte. En **cada entrega** que toque UI, el asistente entrega **prompts de generación de imágenes** listos para usar, y el usuario los coloca en `images/gbe/`. Reglas:

- Estilo base para todos los prompts: *"Granblue Fantasy official art style, painterly anime fantasy, luminous sky, floating islands, sea of clouds, warm golden light, cel-shaded, high detail, Hidetaka Fukuchi / Minaba Hideo inspired"*.
- Nomenclatura: `images/gbe/<seccion>-<detalle>.<ext>` (ej. `images/gbe/fondo-amanecer.jpg`, `images/gbe/skydom-phantagrande.jpg`).
...
- Aspect ratios: fondos sitio 16:9, **banner PJ 16:9**, avatar 3:4 o 1:1, **icono 1:1**, sellos globales 1:1, paneles Skydom 16:9.
...
| Patrón OP | Reskin GBF |
|---|---|
| Bounty Poster (ficha) | **Carta de Skyfarer** (registro del gremio) |
| Bitácora de navegación (posts) | **Diario de vuelo** |
| Mapa del mundo (categorías) | **Carta celeste de Skydoms** |
| Documento del World Government | **Edicto Imperial / Archivo de la Orden** |
| Tablón de contratos (misiones) | **Órdenes de Gremio** |
...
## 5. El mundo: Skydoms y estructura del foro

**Decisiones cerradas (§0.1):** Skydoms **canónicos** (Phantagrande, Nalhegrande, Zeephone, Auguste, Estalucia…), islas **originales** dentro de cada uno.
...
├── EL CIELO (categoría)
│     └── Skydoms como paneles-región (bento, igual que los 8 mares)
│           └── [clic] → forumdisplay del Skydom → islas (subforos)
...
```
GRANBLUE FANTASY: ETERNAL
├── 📰 GACETA DEL CIELO (Noticias / Anuncios / Mundo Vivo)
├── 🌅 [SKYDOM INICIAL] — Zona de inicio (T1)
│     Islas puerto, gremio, tutorial, presentaciones
├── ☁️ [SKYDOM MEDIO] — Intermedio (T2–T3)
│     Islas de conflicto, Primals menores
├── ⚡ [ALTO CIELO] — Avanzado (T4+)
│     Islas legendarias, Primarchs, umbral de Estalucia
├── 🏛️ IMPERIO / ÓRDENES (facciones de poder)
├── 📋 GREMIO DE AVENTUREROS (misiones, combates, torneos)
├── 👤 REGISTRO DE SKYFARERS (fichas, crews, aeronaves)
...
| `hero-mundo.jpg` | ✅ | Slide ambientación / Skydom Phantagrande |
| `hero-pueblos.jpg` | ✅ | Slide pueblos / Skydom Nalhegrande |
| `hero-primal.jpg` | ✅ | Slide primals / Skydom Auguste |
| `cloud-layer-1.png` | ✅ | Reserva (no usado en hero actual) |
| `skydom-zeephone.jpg` | ⏳ | Panel Zeephone |
| `skydom-estalucia.jpg` | ⏳ | Panel Estalucia |
...
| **Erune** | Sentidos agudos, orejas/cola animal | Percepción / Mente |
| **Draph** | Cuernos, fuerza física | Cuerpo / Vigor |
| **Harvin** | Pequeños, longevos, mágicos | Ingenio / Espíritu |
| **Draconic (Dhoromir)** | Sangre de dragón | Afinidad elemental |

> **Cerrado:** las 5 razas de §0.1 están activas. Sin raza Primal jugable.
...
### 6.3 Poder único: **Pacto Primal** (reskin de Fruta del Diablo)

El elemento de **personalización estrella**. Cada PJ puede sellar un **Pacto con una Primal Beast** (menor), que otorga un poder temático único (como una Fruta, pero:
- ligado a un **elemento** (fuego/agua/tierra/viento/luz/oscuridad),
- con **coste de Éter** (EN) al invocar,
...
Los **6 elementos de GBF** con triángulo de ventaja. **Fijo en creación** (§0.1 C4). Mecánica fuerte en combate (§0.1 C3).
...
| Elemento | Acción |
|---|---|
| PV / EN / PA | Se conservan (EN se re-narrativiza como **Éter**) |
...
> **Cerrado (§0.1 D1):** reskin + mecánicas-firma GBF con narrativa apoyada en mecánicas.
...
`rol_mv_*` se conserva. Reskin narrativo: la tensión entre **Astrales** (precursores), **Primal Beasts** y las naciones humanas. Eventos cada X días que abren islas, despiertan Primals, cambian el mapa.

### 8.2 Misiones → Órdenes de Gremio
`tablon-misiones.php` reskineado. Tiers por Skydom. Recompensa en **Rupies** (reemplaza Berries) + reputación de gremio.

### 8.3 Arco maestro
El gancho de largo plazo (equivalente al "One Piece" como misterio): **Estalucia y el legado de los Astrales**. Objetivo mítico end-game del mundo vivo.

---

## 9. Social: crew, nave y renombre

### 9.1 Crew + aeronave (pilar diferencial de GBF)
...
## 10. Arquitectura técnica y purga
...
- B1. Skydoms canónicos vs originales.
- B2. Islas canónicas vs originales.
- B3. Personajes canónicos GBF: ¿existen como lore/NPC o no aparecen?
- B4. Época/punto de partida de la cronología.
- B5. Moneda (Rupies u otra).

**C. Personalización**
- C1. Razas jugables (selección múltiple).
- C2. Sistema de poder único (Pacto Primal / Weapon Grid / Clases / híbrido).
- C3. Elementos: ¿mecánicos o narrativos?
- C4. ¿Elemento fijo o cambiable?

**D. Combate**
- D1. Solo reskin del motor vs añadir mecánicas-firma GBF (elemento+charge/chain).
...
5. **Wizard GBF** (`crear-personaje.php`) con razas/elemento/clase/arma.
